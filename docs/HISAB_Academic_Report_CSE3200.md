...
The HISAB project engages with several complex engineering problems that go beyond routine software development, as characterized by the ABET/WA criteria for engineering complexity. Table 2 summarises how each complexity attribute is manifested in the project.

**Table 2: Complex Engineering Problems (P1–P7)**

| ID | Attribute | Mapping to HISAB | Status |
|---|---|---|---|
| P1 | Depth of knowledge required | HISAB simultaneously draws on six distinct engineering disciplines: (1) **stochastic processes** — first-order Markov Chain transition matrix construction, walk-forward evaluation, and Laplace smoothing for sparse matrices; (2) **signal processing** — 16 kHz audio capture, 80-bin mel spectrogram extraction with 25 ms Hamming window and 10 ms hop, CTC decoding; (3) **computer vision** — Otsu's adaptive binarisation, Hough deskew, two-stage CNN pipeline for line detection and digit recognition; (4) **cybersecurity** — bcrypt KDF (12 rounds), JWT rotation with family-invalidation replay protection, Zod schema validation, rate-limiting, and defence-in-depth layering; (5) **mobile software engineering** — offline-first SQLite architecture, MVVM/Clean Architecture, React Native cross-platform development, TFLite INT8 quantisation; and (6) **HCI for low-literacy contexts** — Universal Design principles, Bangla typography, voice-first interaction patterns for high-noise retail environments. No single subdiscipline is sufficient; all six must be understood and integrated. | Addressed |
| P2 | Range of conflicting requirements | HISAB faces at least four direct requirement conflicts: (a) **Accuracy vs. size vs. latency vs. offline** in the Bengali ASR subsystem — achieving > 90% intent accuracy while remaining < 5 MB and < 1,500 ms on-device places mutually exclusive demands that necessitate domain specialisation and INT8 quantisation rather than a standard ASR model; (b) **Security vs. offline capability** — standard security mandates server-side session validation and continuous token verification, directly conflicting with the requirement for full functionality without network access, resolved via a layered local KDF + JWT + PIN-binding architecture; (c) **Rich feature set vs. minimal cognitive load** — the target user requires a feature-complete retail management system yet cannot navigate multi-level menus or text-heavy screens, resolved through voice-first input, large touch targets, and icon-led navigation; (d) **Analytical transparency vs. data minimisation** — the area-based trending feature requires aggregated sales data from multiple shops, conflicting with privacy-by-design and the legal obligations under Bangladesh's Digital Security Act, resolved through opt-in, anonymisation, and minimum cluster-size gating. | Partial |
| P3 | Depth of analysis required | The Markov Chain demand forecasting engine required rigorous multi-stage analysis: (1) formalisation of demand as a discrete state space {LOW, MEDIUM, HIGH} and derivation of the transition probability matrix from historical weekly sales; (2) design of a leakage-safe walk-forward (rolling-origin) cross-validation protocol to prevent future data contamination of training windows; (3) calibration of eight Bangladesh-specific seasonal multipliers (Ramadan ×1.8, Eid ul-Fitr ×2.0, harvest season ×1.3, rainy season ×0.8, etc.) from cultural and economic domain research; (4) statistical quality assessment via calibration error and discrimination metrics; (5) economic utility evaluation via hit rate and gain/loss ratio; (6) robustness analysis under regime-shift and demand-spike stress scenarios; and (7) design of a multi-tier graceful degradation path (full Markov → smoothed Markov → deterministic reorder engine → safety-net fallback) for shops with insufficient transaction history. The customer trust scoring algorithm additionally required behavioural feature engineering across five financial indicators (outstanding balance, mean repayment latency, late-payment frequency, relationship length, and recent payment activity) with a clamp-bounded score-accumulation classifier. | Addressed |
| P4 | Familiarity of issues | Three sub-problems in HISAB involve genuinely novel or unfamiliar engineering territory with no directly applicable prior solution: (a) **Bengali retail-domain ASR** — no existing lightweight on-device model exists for the closed-vocabulary retail Bengali command space; the CNN + CTC architecture, domain specialisation strategy, and Levenshtein-based name matching pipeline were designed from first principles; (b) **Handwritten baki khata OCR** — no public labeled dataset of Bengali handwritten credit-ledger pages exists; the synthetic data generation pipeline (PIL/Pillow with handwriting-style fonts, augmentation transforms simulating physical damage and paper texture) and the two-stage line-detector + digit-recogniser CNN architecture address a previously unsolved document AI problem; (c) **Offline-first MLOps** — standard MLOps pipelines assume continuous cloud connectivity for model serving, monitoring, and updating; adapting staged rollout (hash-based deterministic assignment), drift detection, and recalibration scheduling to an intermittently connected mobile deployment is an unfamiliar systems challenge without established reference architectures. | Addressed |
| P5 | Extent of applicable codes | The following established codes, standards, and best-practice frameworks are applied across HISAB: **Software Architecture** — R. C. Martin's Clean Architecture [12] and GoF design patterns [13] (Repository, Strategy, Adapter, Observer, Facade, Command, Composite, Decorator); **Process** — Scrum Guide [14] for sprint management and ISO/IEC/IEEE 15288:2015 [15] for system lifecycle process compliance; **Security** — OWASP Top-10 mitigation checklist, IETF RFC 7519 (JSON Web Tokens), NIST password hashing guidance (bcrypt/Argon2), and Helmet.js HTTP security header suite (HSTS, CSP, X-Frame-Options, XCTO, Referrer-Policy); **Database** — ACID transaction principles for all financial mutations and SQLite WAL mode for concurrent read performance; **Accessibility** — WCAG 2.1 Level AA guidelines for touch target sizing and colour contrast applied to low-literacy UI; **Legal/Regulatory** — Bangladesh Bank digital transaction guidelines, Bangladesh Digital Security Act data minimisation and retention obligations, and CC BY 4.0 licensing for planned open dataset release. | Addressed |
| P6 | Extent of stakeholder involvement and conflicting requirements | HISAB must satisfy at least five stakeholder groups with partially incompatible requirements: (1) **Small shop owners** — demand extreme simplicity, Bengali-only UI, offline operation, and voice input; (2) **Credit customers** — demand transparency, dispute resolution capability, and receipt authenticity; (3) **Academic evaluators** — demand architectural rigour, documented design patterns, formal methodology, and research contribution; (4) **Bangladesh Bank / regulators** — impose data retention obligations, digital transaction recording requirements, and potential AML compliance reporting; (5) **Tax authorities** (indirect) — comprehensive digital records could create new VAT/income-tax exposure for shopkeepers previously shielded by informality, creating a stakeholder conflict where the system's transparency benefits shopkeepers relationally but may disadvantage them fiscally. Formal structured interviews and a System Usability Scale (SUS) study with actual shopkeeper representatives were not conducted within the eight-week development timeline, leaving usability assumptions empirically unvalidated. Controlled multi-stakeholder requirement negotiation and a formal conflict-resolution framework remain as future work. | Unaddressed |
| P7 | Interdependence | HISAB's subsystems are tightly coupled across at least five interdependency chains: (1) **Schema → All business screens** — the twelve-table SQLite schema with `user_id` scoping underpins every business module; a schema migration propagates through baki, inventory, sales, cashbook, audit, and sync queue simultaneously; (2) **Authentication → All features** — the hybrid local/online authentication decision logic is a prerequisite gate for every business operation; PIN lock-out, token expiry, and offline session restoration affect all downstream screens; (3) **Trust score → Baki UI** — the customer trust scoring computation (five-feature classifier) directly controls whether the photo-capture requirement is enforced in the baki entry form, coupling the customer analytics layer to the UI interaction model; (4) **Voice FSM → Multiple services** — the voice finite-state machine emits fully formed intents that dispatch simultaneously to BakiService, SalesService, and QueryService; changes to the entity slot structure require coordinated updates in the FSM, each service, and the multi-step review wizard UX; (5) **Markov pipeline → Data, backend, and frontend** — the forecasting subsystem spans SQLite (weekly sales aggregation), the backend training/evaluation/monitoring pipeline, the staged rollout controller, and the StockSuggestionsScreen frontend; a change in state discretisation boundaries invalidates the trained model, all historical evaluation metrics, the context multiplier calibration, and the frontend display logic simultaneously. | Addressed |
